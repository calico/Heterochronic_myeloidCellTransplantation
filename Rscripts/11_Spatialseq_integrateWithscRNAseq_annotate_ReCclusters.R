...
# -------------------- UTILITIES --------------------
clear_reductions <- function(seurat_obj) { seurat_obj@reductions <- list(); seurat_obj }

has_genes <- function(obj, genes) {
  present <- intersect(rownames(obj), genes)
  if (length(present) == 0) warning("None of ", paste(genes, collapse = ", "),
...
integrate_layers_harmony <- function(
  ...
  if (verbose) {
    message(sprintf("[integrate_layers_harmony] %s batches in '%s': %s",
  ...
  if (!all(nonempty)) {
    if (verbose) message("[integrate_layers_harmony] Dropping empty layers: ",
  ...
  if (nbatches >= 2) {
    ...
    warning("[integrate_layers_harmony] Only one batch/layer detected after split; ",
...
rec_sc   <- DietSeurat(rec_sc, assays = "RNA");  rec_sc$project   <- "scRNA"
cosmx_im <- DietSeurat(cosmx_im, assays = "RNA"); cosmx_im$project <- "CosMx"
...
DimPlot(immune_mix, reduction = "umap.harmony",
        group.by = c("project","harmony_clusters"))

# Microglia / immune markers across the co-embedding
FeaturePlot(immune_mix, reduction = "umap.harmony",
